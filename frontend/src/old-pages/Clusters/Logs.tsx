...
function LogEventsTable() {
  ...
  const selectedLogStreamName = useState([
    'app',
    'clusters',
  ...
  const columns = useState(['app', 'clusters', 'logs', 'columns']) || [
    'message',
  ]
  ...
  const {
    ...
    filterProps,
    paginationProps,
  } = useCollection(((events && events.events) as LogEvent[]) || [], {
    filtering: {
  ...
  return (
    <div>
      ...
      >
        <div>{selectedLogStreamName}</div>
        <Button loading={pending} onClick={refresh} iconName="refresh" />
      </div>
      <div
        style={{
        ...
        <Table
          ...
          preferences={
            <CollectionPreferences
              onConfirm={({detail}) => {
                setState(
                  ['app', 'clusters', 'logs', 'columns'],
                  detail.visibleContent,
                )
                setState(
                  ['app', 'clusters', 'logs', 'pageSize'],
                  detail.pageSize,
                )
              }}
              title="Preferences"
...
function StreamList({instanceId}: {instanceId: string}) {
  const logStreams =
    useState(['app', 'clusters', 'logs', 'index', instanceId, 'streams']) || []
  ...
  React.useEffect(() => {
    if (
      ...
      searchParams.get('instance') === instanceId &&
      searchParams.get('filename') &&
      `${ip}.${instanceId}.${searchParams.get('filename')}` !==
        selectedLogStreamName
    ) {
      const selected = getState(['app', 'clusters', 'selected'])
      const logStreamName = `${ip}.${instanceId}.${searchParams.get(
        'filename',
      )}`
...
function LogStreamList() {
  ...
  return (
    <div style={{width: '150px'}}>
      ...
      {logStreamIndex &&
        Object.keys(logStreamIndex)
          .filter(k => k !== instanceId)
          .sort()
          .map(instanceId => (
            <StreamList key={instanceId} instanceId={instanceId} />
          ))}
    </div>
  )
}
