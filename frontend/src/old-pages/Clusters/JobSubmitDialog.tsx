...
function QueueSelect() {
  ...
  const queues =
    getState([...clusterPath, 'config', 'Scheduling', 'SlurmQueues']) || []
  ...
  let queuesOptions = [
    ['[ANY]', '[ANY]'],
    ...queues.map((q: any) => [q.Name, q.Name]),
  ]
  ...
  return (
    <>
      ...
      <Header variant="h4" description="Queue where the job will run.">
        Queue
      </Header>
      <Select
        selectedOption={itemToOption(
          findFirst(queuesOptions, (x: any) => {
            return x[0] === partition
          }) || ['[ANY]', '[ANY]'],
        )}
        onChange={({detail}) => {
  ...
  return (
    <>
      <Alert visible header="Experimental!">
        This provides a basic cost estimate based on the expected job run-time,
        the number of nodes and their instance type. Actual costs will vary
        based on node uptime, storage, and other factors. Please refer to Cost
        Explorer for actual cluster costs.
      </Alert>
        ...
        <FormField errorText={errors}>
          ...
          <SpaceBetween direction="horizontal" size="s" key="command">
            <div style={{flexGrow: 1}}>
              <Input
                onChange={({detail}) => {
                  setState(jobRuntimePath, detail.value)
                }}
                value={jobRuntime}
                inputMode="numeric"
                placeholder={'2.5'}
            ...
            <Button loading={costEstimatePending} onClick={estimateCost}>
              Estimate
            </Button>
          </SpaceBetween>
        </FormField>
        {costEstimate && (
          <FormField>Estimated job cost: ${costEstimate.toFixed(2)}</FormField>
        )}
        {costEstimate && (
          <pre>
            Price ($/h) * Time (h) * NodeCount =&gt; {priceEstimate} *{' '}
            {jobRuntime} * {nodes}
          </pre>
        )}
      </div>
...
export default function JobSubmitDialog({submitCallback}: any) {
  ...
  let isMemBasedSchedulingEnabled =
    useState([
      'clusters',
      'index',
      clusterName,
      ...
      footer={
        <Box float="right">
          <SpaceBetween direction="horizontal" size="xs">
            ...
            <Button loading={submitting} onClick={submitJob}>
              Submit
            </Button>
          </SpaceBetween>
        </Box>
        ...
        <ColumnLayout columns={2} borders="vertical">
          <SpaceBetween direction="vertical" size="xxs" key="job-name">
            ...
            <FormField>
              <Input
                onChange={({detail}) => {
                  setState([...jobPath, 'job-name'], detail.value)
                }}
                value={jobName}
          ...
          <SpaceBetween direction="vertical" size="xxs" key="chdir">
            ...
            <FormField>
              <Input
                onChange={({detail}) => {
                  setState([...jobPath, 'chdir'], detail.value)
                }}
                value={chdir}
          ...
          <SpaceBetween direction="vertical" size="xxs" key="nodes">
            ...
            <FormField>
              <Input
                onChange={({detail}) => {
                  setState([...jobPath, 'nodes'], detail.value)
                }}
                value={nodes}
                inputMode="numeric"
                placeholder="0"
          ...
          <SpaceBetween direction="vertical" size="xxs" key="ntasks">
            ...
            <FormField>
              <Input
                onChange={({detail}) => {
                  setState([...jobPath, 'ntasks'], detail.value)
                }}
                value={ntasks}
                inputMode="numeric"
                placeholder="0"
          ...
          {isMemBasedSchedulingEnabled && (
            <SpaceBetween direction="vertical" size="xxs" key="mem">
              <Header
                variant="h2"
              ...
              <FormField>
                <Input
                  onChange={({detail}) => {
                    setState([...jobPath, 'mem'], detail.value)
                  }}
                  value={mem}
                  inputMode="numeric"
                  placeholder="0"
          ...
          <Toggle checked={wrap} onChange={({detail}) => enableWrap(!wrap)}>
            Run a Command (instead of script)
          </Toggle>
          ...
          <SpaceBetween direction="vertical" size="xxs" key="command">
            <Header
              ...
              description={
                wrap
                  ? 'The command to run as a part of this job.'
            ...
            <FormField>
              <Input
                ...
                value={command}
                placeholder={
                  wrap ? 'sleep 30' : '/home/ec2-user/myscript.sbatch'
                }
              />
            </FormField>
          </SpaceBetween>
