...
function JobStateSelect() {
  ...
  let itemToOption = (item: any) => {
    if (!item) return
    return {label: <div style={{minWidth: '200px'}}>{item}</div>, value: item}
  }
    ...
    <>
      <Select
        ...
        selectedOption={itemToOption(
          findFirst(jobStates, (x: any) => {
            return x === jobState
          }) || ['[ANY]'],
        )}
        onChange={({detail}) => {
          if (detail.selectedOption.value === '[ANY]') clearState(jobStatePath)
          else setState(jobStatePath, detail.selectedOption.value)
...
function QueueSelect() {
  ...
  let itemToOption = (item: any) => {
    ...
    return {
      label: <div style={{minWidth: '200px'}}>{title}</div>,
      value: value,
    }
  }
  ...
  return (
    <>
      <Select
        ...
        onChange={({detail}) => {
          if (detail.selectedOption.value === '[ANY]') clearState(queuePath)
          else setState(queuePath, detail.selectedOption.value)
          refreshAccounting({}, null, true)
        }}
        // @ts-expect-error TS(2322) FIXME: Type '({ label: Element; value: any; } | undefined... Remove this comment to see the full error message
...
function JobStep({step}: any) {
  const reqs = getIn(step, ['tres', 'requested', 'max']) || []
  return (
    <span style={{paddingLeft: '10px'}}>
      {reqs
        .filter((req: any) => req.type !== 'energy')
        .map((req: any) => (
          <span key={req.id} style={{paddingRight: '10px'}}>
            {req.type}: {req.count}{' '}
          </span>
        ))}
    </span>
...
function JobSteps({job}: any) {
  const steps = job.steps || []
  return (
    <div>
      {steps.map((step: any, i: any) => (
        <div key={step} style={{paddingLeft: '10px'}}>
          {i}: <JobStep step={step} />
        </div>
      ))}
    </div>
